...
export const reducer = (state: State, action: Action): State => {
  ...
  if (type === "SET_INPUT_TEXT") {
    return { ...state,loading:action.payload==="" ?false: true, outputText: "", inputText: action.payload };
  }
...
export const useTranslate = () => {
  ...
  const { inputText, outputText, fromLanguage, toLanguage, loading } = state;

  // Functions that update the sate
  const setInputText = (payload: string) =>
    dispatch({ type: "SET_INPUT_TEXT", payload });
  const setOutputText = (payload: string) =>
    dispatch({ type: "SET_OUTPUT_TEXT", payload });
  const switchLanguages = () => dispatch({ type: "SWITCH_LANGUAGES" });
  const setFromLanguage = (payload: FromLangauge) =>
